...
def test_sqlalchemy_wine_repository_can_save_a_wine(session):
    new_wine = (
        WineBuilder()
    ...
    repository = SqlAlchemyWineRepository(session)
...
def test_sqlalchemy_wine_repository_can_list_wines(session):
...
def test_sqlalchemy_wine_repository_can_list_wines_filtered_by_price_range(session):
    _insert_wines(session)
    expected = [
        WineBuilder()
        .with_name("Château YYY 2021")
